...
class _CommunityChatScreenState extends ConsumerState<CommunityChatScreen> {
  ...
  void initState() {
    ...
    _poller = Timer.periodic(const Duration(seconds: 4), (_) {
      if (_selected != null && mounted) _loadMessages(showLoading: false);
    });
  }
  ...
  Future<void> _loadUsers() async {
    try {
      final users = await _service.users(search: _search.text);
      if (!mounted) return;
  ...
  Future<void> _loadMessages({bool showLoading = true}) async {
    ...
    try {
      ...
      if (showLoading) _showError(e.toString());
    }
  }
  ...
  Future<void> _send() async {
    ...
    try {
      final message = await _service.send(selected.id, text);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _editMessage(ChatMessage message) async {
    ...
    try {
      ...
      if (updatedText == null || updatedText.trim() == message.content.trim()) return;
      final selected = _selected;
      if (selected == null) return;

      final updated = await _service.edit(selected.id, message.id, updatedText.trim());
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _deleteMessage(ChatMessage message) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text('The message will be replaced with “This message was deleted” for everyone.'),
        actions: [
          TextButton(
    ...
    try {
      final deleted = await _service.delete(selected.id, message.id);
      if (!mounted) return;
      setState(() {
        final index = _messages.indexWhere((m) => m.id == message.id);
        if (index >= 0) _messages[index] = deleted;
      });
      await _load();
    } catch (e) {
      _showError(e.toString());
  ...
  String _conversationName(ChatConversation conversation) {
    if (conversation.type == 'COMMUNITY') return 'Community Chat';
    ...
    return other?.fullName ?? conversation.name ?? 'Direct Chat';
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_selected == null ? 'Community Chat' : _conversationName(_selected!)),
        actions: [
          IconButton(
            tooltip: 'Find residents, guards and admins',
      ...
      body: _selected == null ? _conversationList() : _chatView(),
    );
  }
  ...
  Widget _conversationList() {
    ...
    if (_error != null && _conversations.isEmpty) {
      return Center(child: Text('Unable to load chat.\n$_error', textAlign: TextAlign.center));
    }
        ...
        children: [
          Card(
            child: ListTile(
              ...
              subtitle: const Text('Chat with residents, guards and administrators'),
              trailing: const Icon(Icons.chevron_right),
              onTap: _openCommunity,
            ),
          ),
          const SizedBox(height: 16),
          const Text('Your conversations', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          if (_conversations.isEmpty)
            const Card(child: Padding(padding: EdgeInsets.all(20), child: Text('No direct conversations yet.'))),
          ..._conversations.map(
            (conversation) => Card(
              child: ListTile(
                leading: CircleAvatar(
                  child: Icon(conversation.type == 'COMMUNITY' ? Icons.groups : Icons.person),
                ),
        ...
        Material(
          ...
          child: InkWell(
            ...
            child: const Padding(
              ...
              child: Row(children: [Icon(Icons.arrow_back), SizedBox(width: 8), Text('All conversations')]),
            ),
          ),
        ),
        Expanded(
          child: RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final canModify = mine && !message.isDeleted;
                return Align(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        onChanged: (_) async {
                          final users = await _service.users(search: _search.text);
                          if (context.mounted) setSheetState(() => _users = users);
                        },
                        decoration: InputDecoration(
                          hintText: 'Search residents, guards or admins',
                        ...
                        child: _users.isEmpty
                            ? Center(
                                child: OutlinedButton.icon(
                                  ...
                                  label: const Text('Search community'),
                                ),
                              )
                            : ListView.builder(
